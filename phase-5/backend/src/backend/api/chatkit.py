        ...
        if input_user_message:
            ...
            if user_text or agent_input:
                try:
                    ...
                    # Inject current time dynamically (not cached at module load)
                    now_pkt = datetime.now(PKT)
                    current_time = f"""**CURRENT TIME:**
{now_pkt.strftime("%A, %B %d, %Y at %I:%M %p")} (Pakistan Time)
"""

                    # Run the agent with MCP tools
                    print(f"🔍 Running agent with input: {enhanced_input[:150]}...")
                    # Prepend current time to the input so agent always has fresh time
                    result = await Runner.run(orchestrator_agent, f"{current_time}\n\n{enhanced_input}", run_config=config)
