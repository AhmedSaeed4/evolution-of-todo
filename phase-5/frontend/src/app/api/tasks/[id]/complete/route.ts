...
// Proxies PATCH requests to backend-api via Dapr sidecar
...
export async function PATCH(
  ...
  try {
    const method = `api/${userId}/tasks/${id}/complete`;
    const daprUrl = `${DAPR_URL}/invoke/backend-api/method/${method}`;

    const response = await fetch(daprUrl, {
      method: 'PATCH', // Backend uses PATCH for toggle complete
      headers: {
    ...
    console.error('Dapr proxy error (PATCH /api/tasks/[id]/complete):', error);
